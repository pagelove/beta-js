...
function stableStepSelector(el, parent) {
    ...
    if (itemprop) {
        ...
        try { if (parent.querySelectorAll(`:scope > ${sel}`).length === 1) return sel; } catch(e) {}
        const tagSel = `${tag}${sel}`;
        try { if (parent.querySelectorAll(`:scope > ${tagSel}`).length === 1) return tagSel; } catch(e) {}
    }
...
function generateSelector() {
    try {
        ...
        Debug.error(Debug.PRIMITIVES, '[PLPrimitives] Failed to generate selector:', error);
...
function htmlToNode(html) {
    try {
        ...
        Debug.error(Debug.PRIMITIVES, '[PLPrimitives] Failed to parse HTML:', error);
        throw error;
    }
}
...
class PLElement {
    ...
    req( method, opts) {
        ...
        const details = {
            ...
            headers: {
                ...
                ...(method !== 'POST' && typeof this.element.etag === 'string' && { 'If-Match': this.element.etag })
            },
            ...opts
        };
    ...
    async #processRequest( request ) {
        ...
        this.element.dispatchEvent( new CustomEvent( 'PLMethodStarted', {
    ...
    async DELETE() {
        ...
        if ( response.ok ) this.element.remove();
            ...
            Debug.warn(Debug.PRIMITIVES, `[PLPrimitives] DELETE request failed with status ${response.status}`);
        }
        return response;
    }
...
class PLDocument {
    ...
    set document( aVal ) {
        ...
        this.#document.addEventListener('PLMethodCompleted', async (event) => {
            ...
            if ( !response.ok ) {
                Debug.error(Debug.PRIMITIVES, `[PLPrimitives] PLMethod ${event.detail.method} failed with status ${event.detail.response.status}`);
            }
        });
    ...
    #observeForETag(element) {
        if (!this.#etagObserver) {
            this.#etagObserver = new IntersectionObserver((entries) => {
